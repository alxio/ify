...
Główną ideą recept grupowych jest działanie jednocześnie na wielu urządzeniach, tak, aby zdarzenia na jednym urządzeniu mogły wpływać na drugie. Zatem komunikacja jest możliwa między instancjami  tej samej rcepty na różnych urządzeniach. Jest ona możliwa na dwa sposoby:
\begin{itemize}
...
internenetowej korzystającej z bazy danych. %% bazującej na chmurze -- modelu przetwarzania danych, gdzie ciężar przetwarzania przenoszony jest na serwer.

W kontekście powyższych pomysłów oraz poprzez analogie między receptami a samodzielnymi aplikacjami, zaprojektowano targowisko. Targowisko to aplikacja internetowa
będąca docelowo mniejszym odpowiednikiem Google Play -- internetowego sklepu z aplikacjami na platformę android. Pomysł targowiska zawiera parę innowacji w
porównaniu do Google Play.

Pierwszą z nich jest stworzenie zintegrowanego środowiska programistycznego wbudowanego bezpośrednio w targowisko. Drugą, zastosowanie idei rozwidlania propagującą
pomysł nauki przez przykłady.


\subsection{Zintegrowane środowisko programistyczne}
Współcześnie, wiele złożonych usług dostępnych jest z poziomu przeglądarki internetowej. Podejście takie wiąże się z pojęciem chmury -- modelu przetwarzania
danych, gdzie ciężar przetwarzania przenoszony jest na serwer. O chmurze jako bazie dla Google Play, Jamie Rosenberg napisał w \cite{googleplay}. W targowisku
postanowiono wykorzystać ten model. Właśnie dlatego, zintegrowane środowisko programistyczne postanowiono wbudować w aplikację internetową.
...
oraz skryptowy język PHP który świetnie radzi sobie również z zadaniami niskiego poziomu. %TODO:more?
...
\end{verbatim}
\subsection{Skrypt BASH}       %TODO: rename or drop or move somewhere !
TODO: przedmowa
\begin{verbatim}
#!/bin/bash
...
Dla lepszego wyobrażenia możliwości oferowanych przez framework, wystarczy spojrzeć na rysunek \ref{fig:market}. %TODO: wiecej ?
\begin{figure}[p]
  \centering
  ...
  \label{fig:market}
\end{figure}
\subsection{Edytor Ace}         %TODO: below: czy eclipse można nazwać edytorem?
W celu umożliwienia pisania kodu recept z poziomu targowiska, skorzystano z darmowego edytora Ace. Jest to narzędzie dające możliwości znane z popularnych edytorów
...
\begin{itemize}
\item http://ify.cs.put.poznan.pl/\~{}scony/marketify/api/new.php?limit=\{limit\}
\item \{limit\}, liczba naturalna -- ilość recept do pobrania
\end{itemize}
\item O wszystkie recepty
\begin{itemize}
\item http://ify.cs.put.poznan.pl/\~{}scony/marketify/api/recipes.php?page=\{page\}\&limit=\{limit\}
...
\begin{itemize}
\item http://ify.cs.put.poznan.pl/\~{}scony/marketify/api/search.php?phrase=\{phrase\}
\item \{phrase\}, łańcuch znaków -- wzorzec wyszukiwania
\end{itemize}
\item O konkretną receptę
\begin{itemize}
\item http://ify.cs.put.poznan.pl/\~{}scony/marketify/api/recipe.php?name=\{name\}
\item \{name\}, łańcuch znaków -- pełna nazwa recepty
\end{itemize}
\item O losową receptę
\begin{itemize}
\item http://ify.cs.put.poznan.pl/\~{}scony/marketify/api/random.php
\end{itemize}
\end{itemize}
...
[
   {
      ...
      "ts":1391197304,
      "safe":1,
      "url":"http:\/\/ify.cs.put.poznan.pl\/~scony\/marketify\/jar\/YSampleYRC.jar",
      "rate":null,
      "comments":[
...
\section{Baza danych}
% czyli istnieje relacja między krotką w tabeli groupspermissions i krotkami w tabelach users i groups, 
...
Serwer pośdenicząc w komunikacji odbiera wiadomości, które do czasu wysłania do klienta docelowego są zapisywane w bazie danych.
...
Drugą możliwością jest podanie w polu "target" ciągu "BROADCAST", oznacza on wysłanie identycznej wiadomości do wszystkich członków grupy jaka została podana przy komunikacji. 
...
\subsection{Komunikacja recept z Serwerem Grup}
