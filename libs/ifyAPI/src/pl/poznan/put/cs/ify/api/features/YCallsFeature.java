...
import pl.poznan.put.cs.ify.api.core.PhoneStateReceiver;
import pl.poznan.put.cs.ify.api.core.PhoneStateReceiver.OnPhoneStateChangedListener;
...
/**
 * Feature for listening to calls and discarding it.
 */
public class YCallsFeature extends YFeature {
	...
	private OnPhoneStateChangedListener mListener = new OnPhoneStateChangedListener() {
		...
		public void onReceive(Intent i) {
			...
			for (String s : keySet) {
				Log.d("TELEPHONY", i.getExtras().get(s) + " nazwa dla Olka "
						+ s + i.getExtras().get(s).getClass().getName());
			}
			YCallsEvent event = new YCallsEvent(i.getExtras()
					.getString("state"), i.getExtras().getString(
					"incoming_number"));
	...
	protected void init(IYRecipeHost srv) {
		...
		try {
			...
			mEndCall = mTelephonyService.getClass()
					.getDeclaredMethod("endCall");
			mSilenceRinger = mTelephonyService.getClass().getDeclaredMethod(
					"silenceRinger");
		} catch (Exception e) {
			e.printStackTrace();
		}
